<x-app-layout>
    <x-slot name="header">
        <h1 class="cy-page-title">Editar entrada de bitácora</h1>
        <p class="cy-page-subtitle">{{ $bitacora->fecha->format('d/m/Y') }}</p>
    </x-slot>

    <div class="py-8 max-w-3xl mx-auto safe-px">
        <div class="cy-card p-6">
            <form method="POST" action="{{ route('bitacoras.update', $bitacora) }}" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select id="actividad_id" name="actividad_id" class="cy-select mt-1">
                        ...
                        @foreach($actividades as $act)
                            <option value="{{ $act->id }}"
                                    @selected(old('actividad_id', $bitacora->actividad_id) == $act->id)>
                ...
                <div>
                    ...
                    <input id="fecha" type="date" name="fecha"
                           value="{{ old('fecha', $bitacora->fecha->toDateString()) }}"
                ...
                <div>
                    ...
                    <textarea id="contenido" name="contenido" rows="6"
                              class="cy-input mt-1">{{ old('contenido', $bitacora->contenido) }}</textarea>
                    @error('contenido')
                        <p class="cy-error">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea id="aprendizajes" name="aprendizajes" rows="4"
                              class="cy-input mt-1">{{ old('aprendizajes', $bitacora->aprendizajes) }}</textarea>
                    @error('aprendizajes')
                        <p class="cy-error">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex justify-end gap-3 pt-2">
                    <a href="{{ route('bitacoras.show', $bitacora) }}" class="cy-btn-ghost">Cancelar</a>
